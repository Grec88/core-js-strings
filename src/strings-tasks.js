...
function countVowels(str) {
  ...
  for (let i = 0; i <= resStr.length; i += 1) {
    if (vowels.includes(resStr.at(i))) {
      result += 1;
    }
  }
  return result;
...
function isPalindrome(str) {
  ...
  return (
    spaceLessStr.toLowerCase().split('').reverse().join('') ===
    spaceLessStr.toLowerCase()
  );
}
  ...
  return str
    ...
    .map((char) => {
      if (
        ...
        ((char.charCodeAt() > 'm'.charCodeAt() &&
          char.toLowerCase() === char) ||
          (char.charCodeAt() > 'M'.charCodeAt() && char.toUpperCase() === char))
      ) {
...
function getCardId(value) {
  ...
  return suitIndex === 0 ? deckIndex : deckIndex + 13 * suitIndex;
}
